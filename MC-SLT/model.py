...
class SignModel(nn.Module):
    ...
    def forward(
        ...
        #encoder_output = self.sgn_embed(x=sgn, mask=sgn_mask)
    ...
    def get_loss_for_batch(
        ...
        if self.do_translation:
            ...
            try:
                ...
                s1 = txt_log_probs.size()

                aa = txt_log_probs.view(-1, s1[-1])
                bb = batch.txt.view(-1, 1)

                cc = torch.zeros(aa.size()).cuda().scatter_(1, bb, 1)

                one_hot = cc.view(s1[0], s1[1], s1[2])

                mul = -txt_log_probs * one_hot

                fg_loss = torch.sum(mul, [-1])

                tr_loss = translation_loss/ (s1[0] * s1[1])


                for name, param in self.named_parameters():
                    if name != 'decoder.output_layer.weight':
                ...
                #gl_grad = torch.autograd.grad(tr_loss, filter(lambda p: p.requires_grad, self.parameters()),
                #                              create_graph=True, allow_unused=True)



                #gl_grad2 = torch.mean(gl_grad[0], 0)
                ...
                for i in range(s1[0]):
                    for j in range(s1[1]):
                        #at_grad = torch.autograd.grad(fg_loss[i][j], filter(lambda p: p.requires_grad, self.parameters()), create_graph=True, allow_unused=True)


                        #at_grad2 = (self.l1(at_grad[0])).transpose(0, 1)

                        #at_grad2 = (torch.sum(torch.abs(at_grad[0]), 0)).unsqueeze(0)

                        auto_grad.append((self.l1(torch.autograd.grad(fg_loss[i][j], filter(lambda p: p.requires_grad, self.parameters()), create_graph=True, allow_unused=True)
[0].detach())).transpose(0, 1))

                        #embed()

                        #auto_grad.append(at_grad2)

                        #auto_grad.append(at_grad[0].unsqueeze(0))

                #from IPython import embed
                #embed()


                auto_grad2 = torch.cat(auto_grad, 0).view(s1[0], s1[1], -1)


                #auto_grad2 = auto_grad2.detach()

                #print (auto_grad2.size())
                #embed()

                auto_sig = self.sig(self.l2(auto_grad2)) * 2

                #auto_sum = torch.sum(auto_sig, 1).unsqueeze(1)

                #auto_sig = auto_sig / auto_sum * s1[1]
                ...
                txt_log_probs2 = txt_log_probs * auto_sig
                #txt_log_probs *= 2
            ...
            #auto_grad.append(torch.cat(at_grad, gl_grad))
