...
class TrainManager:
    ...
    def __init__(self, model: SignModel, config: dict) -> None:
        ...
        #model_path = "/data2/jintao/slt/slt-master/sign_sample_model3/3300.ckpt"
    ...
    def train_and_validate(self, train_data1: Dataset, train_data2: Dataset, train_data3: Dataset, train_data4: Dataset, train_data5: Dataset, train_data6: Dataset, train_data7: Dataset, train_data8: Dataset, valid_data: Dataset) -> None:
        """
        ...
        iternum = max(len(train_iter1), len(train_iter2), len(train_iter3), len(train_iter4), len(train_iter5), len(train_iter6), len(train_iter7), len(train_iter8))


        t_it = [train_iter1, train_iter2, train_iter3, train_iter4, train_iter5, train_iter6, train_iter7, train_iter8]
        #embed()
        ...
        for epoch_no in range(self.epochs):
            ...
            for i in range(iternum):
                ...
                try:
                    ...
                    aaa6 = iter(train_iter6)
                    batch6 = next(aaa6)

                try:
                    batch7 = next(aaa7)
                except:
                    aaa7 = iter(train_iter7)
                    batch7 = next(aaa7)

                try:
                    batch8 = next(aaa8)
                except:
                    aaa8 = iter(train_iter8)
                    batch8 = next(aaa8)
    ...
    def _train_batch(self, batch: list(), update: bool = True) -> (Tensor, Tensor):
        ...
        meta_test_set = [batch[4], batch[5], batch[6], batch[7]]
        ...
        for n_scr in range(len(adapted_state_dicts)):
            ...
            if self.do_translation:
                if self.translation_normalization_mode == "batch":
                    txt_normalization_factor = meta_test_set[n_scr].num_seqs
                elif self.translation_normalization_mode == "tokens":
                    txt_normalization_factor = meta_test_set[n_scr].num_txt_tokens
                else:
                    raise NotImplementedError("Only normalize by 'batch' or 'tokens'")
        ...
        if self.do_recognition:
            self.total_gls_tokens += meta_test_set[n_scr].num_gls_tokens
        if self.do_translation:
            self.total_txt_tokens += meta_test_set[n_scr].num_txt_tokens
...
def train(cfg_file: str) -> None:
    ...
    train_data, dev_data, test_data, gls_vocab, txt_vocab, pos_vocab, s1_data, s2_data, s3_data, s4_data, s5_data, s6_data, s7_data, s8_data = load_data(
        data_cfg=cfg["data"]
    )
    ...
    #pre_embed = "/data2/jintao/slt/pre_embed.npy"
    #import numpy as np

    #pre_emb = np.load(pre_embed)
    #model.txt_embed.lut.weight.data.copy_(torch.from_numpy(pre_emb))
    ...
    trainer.train_and_validate(train_data1=s1_data, train_data2=s2_data, train_data3=s3_data, train_data4=s4_data, train_data5=s5_data, train_data6=s6_data, train_data7=s7_data, train_data8=s8_data, valid_data=dev_data)

    # Delete to speed things up as we don't need training data anymore
    del train_data, dev_data, test_data, s1_data, s2_data, s3_data, s4_data, s5_data, s6_data, s7_data, s8_data
